...
cd ${CURR_DIR} && \
for N in ${nodes[@]}; do
	sbatch -N ${N} -t 12:00:00 bdw-strong-scaling.job ${ROOT_DIR}/app/BSPMM/bspmm_profile_original bspmm_profile_original.out ${N}
	stats=$?
	if [ $stats != 0 ]; then
...
cd ${CURR_DIR} && \
for N in ${nodes[@]}; do
	sbatch -N ${N} -t 12:00:00 bdw-strong-scaling.job ${ROOT_DIR}/app/BSPMM/bspmm_profile_throughput bspmm_profile_throughput.out ${N}
	stats=$?
	if [ $stats != 0 ]; then
